...
class NavBar extends React.Component {
    constructor(props) {
        super(props);
        this.state = { navItems: siteNavItems };
    }

    render() {
        var mapNavItems = this.state.navItems.map(function iterator(navItem) {
            return (
                <NavItem key={navItem.id} title={navItem.title} link={navItem.link} />
            );
        });

        return (
            <div className="top-bar">
                <ul className="menu">{mapNavItems}</ul>
            </div>
        );
    }
}
